  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productData = {
        ...
        artisan_name: user.name || user.displayName || 'Anonymous Artisan',
        artisan_id: user.uid,
        images: images.map(img => img.file), // Pass File objects directly for processing
        rating: 0,
        reviewCount: 0,
        // Include generated video if available
        ...(generatedVideo && { 
      ...
      if (error.message) {
        if (error.message.includes('PERMISSION_DENIED')) {
          errorMessage = 'Firestore permission denied. Please check your Firestore security rules.';
